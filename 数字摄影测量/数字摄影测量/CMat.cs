...
namespace 数字摄影测量
{
    class CMat
    {
        ...
        //累加矩阵
        public double[,] MatAddTo(double[,] a, double[,] b, int m, int n)
        {
            double[,] result = b;
            ...
            for (i = 0; i < i1; i++)
            {
                for (j = 0; j < j1; j++)
                {
                    result[i + m, j + n] = result[i + m, j + n] + a[i, j];
                }
            }
            return result;
        }

        public double[] MatAddTo(double[] a, double[] b, int m)
        {
            double[] result = b;
            int i1 = a.GetLength(0);
            int i;
